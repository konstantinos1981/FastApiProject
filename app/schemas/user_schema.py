...
class UserCreate(UserBase):
    ...
    model_config = ConfigDict(
        from_attributes=True, json_schema_extra={"example": user_create_example}
    )
...
class UserUpdate(BaseModel):
    ...
    model_config = ConfigDict(
        from_attributes=True, json_schema_extra={"example": user_update_example}
    )
    ...
    model_config = ConfigDict(
        {
            ...
            "json_schema_extra": {"example": user_read_example},
        }
    )
...
class UserPasswordUpdate(BaseModel):
    ...
    model_config = ConfigDict(
        ...
        json_schema_extra={"example": user_password_change_example},
    )

    @field_validator("confirm_password")
    def passwords_match(cls, v, info):
        return confirm_field_match(
            v, info.data, "new_password", "Passwords do not match."
        )
...
class UserWithTodos(UserRead):
    ...
    model_config = ConfigDict(
        {
            "from_attributes": True,
            "json_schema_extra": {"example": user_with_todos_example},
        }
    )
