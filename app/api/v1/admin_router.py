...
def get_user(db: db_dependancy, username:str|None = None, email: EmailStr|None = None ) -> User:
    if email:
    ...
    if username:
        ...
        return user
    raise HTTPException(
        status_code=status.HTTP_400_BAD_REQUEST, detail="Either username or email must be provided"
    )
